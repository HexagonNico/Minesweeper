...
import minesweeper.utils.MapGenerator;
...
public class GameBoard extends JPanel implements MouseListener, ActionListener {
	...
	/**Creates a GameBoard object and initializes class fields*/
	...
	public void paintComponent(Graphics g) {
		...
		if(detectHasLost()) {
			...
			g.drawString("YOU LOST!",  DisplayManager.getWindowWidth()/5, DisplayManager.getWindowHeight()/7);
		}
		
		if(detectHasWon()) {
			g.setFont(new Font("Arial", Font.BOLD, 40));
			g.setColor(Color.BLUE);
			g.drawString("YOU WON!",  DisplayManager.getWindowWidth()/5, DisplayManager.getWindowHeight()/7);
		}
	}
